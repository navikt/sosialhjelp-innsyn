...
const handler = async (req: NextApiRequest, res: NextApiResponse<unknown>) => {
    ...
    console.log(process.env.NEXT_PUBLIC_RUNTIME_ENVIRONMENT);
    ...
    if (!token) {
        console.log("Fant ikke token, Redirecter til login");
        return res.redirect("/oauth2/login");
    }
    console.log("Fant token: ", token);
    if (!slug) {
    ...
    console.log("Redirecter til ", path);
    console.log("På hostname ", process.env.NEXT_INNSYN_API_HOSTNAME);
