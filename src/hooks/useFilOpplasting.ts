...
const useFilOpplasting = (
        ...
        (index: number, _files: File[]) => {
            ...
            _files.forEach((file) => {
                if (file.size > maxFileSize) {
                    _errors.push({fil: file, errorType: Feil.FILE_TOO_LARGE});
                }
                if (containsIllegalCharacters(file.name)) {
                    _errors.push({fil: file, errorType: Feil.ILLEGAL_FILE_NAME});
                }
            });

            if (_files.concat(files[index]).reduce((acc, curr) => acc + curr.size, 0) > maxCombinedFileSize) {
                _errors.push({errorType: Feil.COMBINED_TOO_LARGE});
            }
    ...
    const upload = useCallback(() => {
        if (allFiles.length === 0) {
            ...
            setOuterErrors([{errorType: Feil.NO_FILES}]);
            return;
        }
            ...
            },
            {
                ...
                onSuccess: async (data, variables, context) => {
                    ...
                    const errors: Error[] = filerData
                        ...
                        .map((it) => ({errorType: determineErrorType(it.status)!, filnavn: it.filnavn}));
                ...
                onError: (error, variables, context) => {
                    ...
                    if (error.message === "Mulig virus funnet") {
                        ...
                        setOuterErrors([{errorType: Feil.VIRUS}]);
                    } else {
                        fileUploadFailedEvent(errorStatusToMessage[Feil.KLIENTFEIL]);
                        setOuterErrors([{errorType: Feil.KLIENTFEIL}]);
                    }
                },
            }
        );
