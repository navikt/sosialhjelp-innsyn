...
export async function middleware(request: NextRequest) {
    ...
    if (process.env.NEXT_USE_WONDERWALL === "false") {
        try {
            logger.debug("Kaller /tilgang i innsyn-api");
            const harTilgangResponse = await fetch(process.env.NEXT_INNSYN_API_BASE_URL + "/api/v1/innsyn/tilgang", {
                headers: new Headers(request.headers),
                credentials: "include",
            });
            logger.debug(`Fikk ${harTilgangResponse.status} på kall til /tilgang`);
            if (harTilgangResponse.status === 401) {
                ...
                logger.debug(`Sender bruker til login: ${json.loginUrl + queryDivider + redirectUrl}`);
        ...
        logger.debug("Wonderwall er skrudd på. Skipper sjekk mot /tilgang");
    }
}
