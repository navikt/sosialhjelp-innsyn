...
const pageHandler = async (
    ...
    if (tilgangStatus === 401) {
        ...
        if (isMock) {
            return {
                redirect: {
                    destination: `${process.env.NEXT_INNSYN_MOCK_LOGIN_URL}?redirect=http://localhost:3000/sosialhjelp/innsyn${context.resolvedUrl}`,
                    permanent: false,
                },
            };
        }
        return {
            redirect: {
                destination: `/oauth2/login?redirect=/sosialhjelp/innsyn${context.resolvedUrl}`,
                permanent: false,
            },
        };
    }
