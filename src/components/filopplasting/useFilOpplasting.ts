...
import {logDuplicatedFiles, loggFileUploadFailedEvent} from "../../utils/amplitude";
        ...
        mutate(
            ...
            },
            {
                ...
                onError: (error, variables, context) => {
                    ...
                    loggFileUploadFailedEvent("vedlegg.opplasting_feilmelding");
                    logger.warn("Feil med opplasting av vedlegg: " + error.message);
                    if (error.message === "Mulig virus funnet") {
                        loggFileUploadFailedEvent(errorStatusToMessage[Feil.VIRUS]);
                        setOuterErrors([{feil: Feil.VIRUS}]);
                    } else {
                        loggFileUploadFailedEvent(errorStatusToMessage[Feil.KLIENTFEIL]);
