...
const Tilgangskontrollside: React.FC<TilgangskontrollsideProps> = ({children, queryHas403}) => {
    ...
    useEffect(() => {
        console.log("status: ", sessionQuery.data?.status, ", active: ", sessionQuery.data?.active);
        if (!sessionQuery.isPending && (sessionQuery.data?.status === 401 || sessionQuery.data?.active === false)) {
